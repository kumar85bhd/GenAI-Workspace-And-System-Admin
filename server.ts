...
async function startServer() {
  ...
  app.get("/api/workspace/apps", (req, res) => {
    const apps = db.prepare(`
      SELECT 
        a.id, a.name, a.slug, a.icon, a.url, a.description as desc, 
        a.display_order, a.is_active, c.name as category 
  ...
  app.post("/api/workspace/apps", authenticateToken, isAdmin, (req, res) => {
    const { name, slug, category_id, icon, url, description, display_order } = req.body;
    try {
      const info = db.prepare('INSERT INTO workspace_apps (name, slug, category_id, icon, url, description, display_order) VALUES (?, ?, ?, ?, ?, ?, ?)')
        .run(name, slug, category_id, icon, url.toString(), description, display_order || 0);
