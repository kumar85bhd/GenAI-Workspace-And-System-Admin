...
## 2. Architecture
The application follows a full-stack architecture with a modular monolith frontend built with React, Vite, and Tailwind CSS, and a backend built with FastAPI (Python) and a PostgreSQL database.

### 2.1 Modules
- **Backend (`/backend`)**: FastAPI server handling authentication, authorization, and API routes. It uses SQLAlchemy as an ORM to interact with the PostgreSQL database.
...
- **Authentication**: Managed via a FastAPI backend using JWT (JSON Web Tokens) with HS256 signature verification.
- **Authorization**: Role-based access control (RBAC).
  - `user`: Access to Workspace.
  - `admin`: Access to Workspace and Admin Dashboard.
- **Route Protection**: The `AdminRoute` component guards the `/admin` routes on the frontend, and the `get_current_admin_user` dependency guards API routes on the backend.

## 3. Data Storage
- **Database**: A PostgreSQL database is used as the single source of truth for all data, including users, workspace apps, and categories.
