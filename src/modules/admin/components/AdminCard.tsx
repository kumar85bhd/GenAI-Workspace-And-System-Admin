...
import { ExternalLink, RefreshCw, Database, Server, Globe, Box, Activity, Link as LinkIcon, BarChart } from 'lucide-react';
...
const getServiceIcon = (type: string) => {
  ...
  if (t.includes('db') || t.includes('database')) return <Database size={18} />;
  if (t.includes('api') || t.includes('web') || t.includes('globe')) return <Globe size={18} />;
  if (t.includes('server') || t.includes('backend')) return <Server size={18} />;
  if (t.includes('link') || t.includes('external')) return <LinkIcon size={18} />;
  if (t.includes('activity') || t.includes('monitor')) return <Activity size={18} />;
  if (t.includes('chart') || t.includes('powerbi')) return <BarChart size={18} />;
  return <Box size={18} />;
};
    ...
    >
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              <span className="text-[10px] text-slate-500 dark:text-slate-400 font-mono uppercase tracking-widest bg-slate-100 dark:bg-slate-700/50 px-1.5 py-0.5 rounded">
                {service.type}
              </span>
      ...
      <div className="mt-auto space-y-3 relative z-10">
        {metrics.length > 0 ? (
          ...
          <div className="bg-slate-50/50 dark:bg-slate-900/30 rounded-xl p-4 border border-slate-100/50 dark:border-slate-700/30 flex items-center justify-center gap-3 text-slate-400">
            <Activity size={16} className="animate-pulse" />
            <span className="text-xs font-semibold tracking-wide uppercase">Monitoring Active</span>
          </div>
        )}
      </div>
    </div>
